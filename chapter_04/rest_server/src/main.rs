...
async fn return_error(rejection: Rejection) -> Result<impl Reply, Rejection> {
    ...
    } else if let Some(_invalid_id) = rejection.find::<InvalidId>() {
        Ok(warp::reply::with_status(
...
async fn main() {
    ...
    // To test: curl -X OPTIONS localhost:3030/questions -H "Access-Control-Request-Method: PUT" -H "Access-Control-Request-Headers: content-type" -H "Origin: https://not-origin.io" -verbose
